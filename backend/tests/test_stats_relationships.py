...
async def test_pairs_fit_and_points(client, session, clean_db, tmp_path):
    ...
    response = await client.get(
        "/stats/relationships", params={"agg": "pairs", "x": "co2", "y": "air_temperature"}
    )
...
async def test_pairs_sampling_is_deterministic(client, session, clean_db, tmp_path, monkeypatch):
    ...
    first = (await client.get("/stats/relationships", params=params)).json()["buckets"][0]
    from api.services.explore.cache import cache

    cache.clear()
    second = (await client.get("/stats/relationships", params=params)).json()["buckets"][0]
...
async def test_matrix_pearson_and_spearman(client, session, clean_db, tmp_path):
    ...
    for method, expected_r in (("pearson", regression(points)["r"]), ("spearman", spearman(points))):
        response = await client.get(
            "/stats/relationships",
            params={"agg": "matrix", "parameters": "co2,air_temperature,pm2_5", "method": method},
        )
        assert response.status_code == 200, response.text
    ...
    response = await client.get("/stats/relationships", params={"agg": "matrix", "parameters": "co2"})
    assert response.status_code == 422
    response = await client.get("/stats/relationships", params={"agg": "pairs", "x": "co2"})
    assert response.status_code == 422
