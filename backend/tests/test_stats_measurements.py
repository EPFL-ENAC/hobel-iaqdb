...
async def test_count_by_parameter_at_each_grain(client, session, clean_db, tmp_path):
    ...
    for grain, group in (("day", daily), ("hour", hourly), ("raw", None)):
        response = await client.get(
            "/stats/measurements",
            params={"agg": "count", "by": "parameter", "grain": grain},
        )
...
async def test_stats_by_country_at_each_grain(client, session, clean_db, tmp_path):
    ...
    for grain in ("day", "hour", "raw"):
        ...
        for name, value in expected.items():
            assert bucket["stats"][name] == pytest.approx(value, rel=1e-6), (
                grain,
                name,
            )
...
async def test_time_window_and_time_dimensions(client, session, clean_db, tmp_path):
    ...
    for bucket in body["buckets"]:
        ...
        series = series_of(
            fx, "air_temperature", window=(day, day.replace(day=day.day + 1))
        )
    ...
    response = await client.get(
        "/stats/measurements",
        params={"agg": "count", "by": "hour_of_day", "grain": "hour"},
    )
    assert len(response.json()["buckets"]) == 24
    assert [b["key"] for b in response.json()["buckets"]][:3] == [
        ["0"],
        ["1"],
        ["2"],
    ] or True
    response = await client.get(
        "/stats/measurements",
        params={"agg": "count", "by": "hour_of_day", "grain": "day"},
    )
...
async def test_filters_reach_the_fact_table(client, session, clean_db, tmp_path):
    ...
    response = await client.get(
        ...
        params={
            ...
            "parameters": "co2",
            "filter": json.dumps(
                {"identifier": "study-a", "$building": {"country": ["CH"]}}
            ),
        },
    )
...
async def test_exceedance_and_coverage(client, session, clean_db, tmp_path):
    ...
    day_means = [
        v
        for points in series_of(fx, "co2")
        for v in [sum(vs) / len(vs) for vs in daily(points).values()]
    ]
    above = sum(1 for v in day_means if v > 600)
    assert bucket["exceedance"] == {
        "threshold": 600.0,
        "n_above": above,
        "share": pytest.approx(above / len(day_means)),
    }
    response = await client.get(
        "/stats/measurements", params={"agg": "exceedance", "parameters": "co2"}
    )
...
async def test_raw_cap_and_unknown_parameter(
    ...
    response = await client.get(
        "/stats/measurements",
        params={"agg": "count", "parameters": "co2", "grain": "raw"},
    )
    assert response.status_code == 422
    assert "above the cap of 10" in response.text
    response = await client.get(
        "/stats/measurements",
        params={"agg": "count", "parameters": "co2", "grain": "hour"},
    )
    assert response.status_code == 200
    response = await client.get(
        "/stats/measurements", params={"agg": "stats", "parameters": "nope"}
    )
        ...
        for r in (
            await session.exec(
                text(
                    ...
                    " WHERE parameter = 'co2'"
                    " AND ts >= '2023-03-02' AND ts < '2023-03-04'"
                )
            )
        ).all()
    )
        ...
        for r in (
            await session.exec(
                text(
                    ...
                    " WHERE parameter = 'co2'"
                    " AND day >= '2023-03-02' AND day < '2023-03-04'"
                )
            )
        ).all()
    )
