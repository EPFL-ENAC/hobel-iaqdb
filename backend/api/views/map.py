...
async def getBuildings(
    ...
    studyColorsDict = {
        study.id: {"study": study, "color": string_to_color(study.identifier)}
        for study in studyResults.data
    }
    # aggs = Room.aggregate({"$group": {"_id": {"building": "$building",
    #   "ventilation": "$ventilation"}, "count": {"$sum": 1}}})

    features = []
    for building in buildingsResult.data:
        # roomAggs = await Room.find(Room.building.id == building.id).aggregate(
        #   [{"$group": {"_id": "$ventilation", "count": {"$sum": 1}}}]).to_list()
        # "|".join(map(lambda agg: agg["_id"],
        #   filter(lambda agg: agg["count"] > 0, roomAggs)))
        mechanical_ventilation_types = "|".join(
            set(
                [
                    space.mechanical_ventilation_type
                    for space in building.spaces
                    if space.mechanical_ventilation_type is not None
                ]
            )
        )
        if building.long is None or building.lat is None:
            continue
        geometry = Geometry(coordinates=[building.long, building.lat], type="Point")
...
async def getClimateZone(
    lon: float = Query(0), lat: float = Query(0), precise: bool = Query(False)
) -> ClimateZone:
    service = GeoService()
