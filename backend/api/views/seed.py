...
async def seed(session: AsyncSession = Depends(get_session)) -> SeedStatus:
    ...
    occupant_impacts = ["health", "comfort", "performance", "well-being", "other"]
    special_populations = [
        "low-income",
        "middle-income",
        "elderly",
    ...
    ventilation_types = ["dilution", "displacement", "exhaust only", "other", "unknown"]
    ...
    for i in range(0, 3):
        study = Study(
            ...
            occupant_impact=fake.word(ext_word_list=occupant_impacts),
            other_indoor_param=fake.word(ext_word_list=other_indoor_params),
        ...
        for j in range(0, 3):
            ...
            for k in range(0, random.randint(1, 5)):
                parameter = InstrumentParameter(
                    physical_parameter=fake.word(ext_word_list=physical_params),
        ...
        for j in range(0, 3):
            ...
            renovation_year = (study.start_year + 9) if renovation == "yes" else None

            special_population = fake.word(ext_word_list=special_populations)
            ...
            building = Building(
                ...
                outdoor_env=fake.word(ext_word_list=outdoor_envs),
                construction_year=(study.start_year - 1),
                renovation=renovation,
                renovation_year=renovation_year,
                mechanical_ventilation=fake.word(ext_word_list=yes_no),
                operable_windows=fake.word(ext_word_list=yes_no),
            ...
            for k in range(0, 10):
                ...
                major_combustion_sources = (
                    fake.word(ext_word_list=major_combustion_sources)
                    if combustion_sources == "yes"
                    else None
                )
                minor_combustion_sources = (
                    fake.word(ext_word_list=minor_combustion_sources)
                    if combustion_sources == "yes"
                    else None
                )

                space = Space(
                    identifier=f"seed-{j}-{k}",
                    type=fake.word(ext_word_list=space_types),
                    occupancy=fake.word(ext_word_list=occupancy_status),
                    mechanical_ventilation_type=fake.word(
                        ext_word_list=ventilation_types
                    ),
                    mechanical_ventilation_status=fake.word(ext_word_list=on_off),
                    ...
                    cooling_type=fake.word(ext_word_list=cooling_types),
                    cooling_status=fake.word(ext_word_list=on_off),
                    heating_type=fake.word(ext_word_list=heating_types),
