...
class BuildingQueryBuilder(QueryBuilder):
    ...
    def build_query_with_joins(self, total_count, filter):
        ...
        query = self._apply_joins(query, filter)
        query = query.options(
            selectinload(Building.certifications),
...
class BuildingService:
    ...
    async def count(self) -> int:
        """Count all buildings"""
        count = (
            await self.session.exec(text("select count(id) from building"))
        ).scalar()
        return count
    ...
    async def get(self, building_id: int) -> Building:
        ...
        res = await self.session.exec(
            ...
            .options(
                selectinload(Building.certifications), selectinload(Building.spaces)
            )
        )
    ...
    async def find(self, filter: dict, sort: list, range: list) -> BuildingsResult:
        ...
        builder = BuildingQueryBuilder(
            Building, filter, sort, range, {"$study": Study, "$space": Space}
        )
