...
class StudyQueryBuilder(QueryBuilder):
    ...
    def build_query_with_joins(self, total_count, filter):
        ...
        query = self._apply_joins(query, filter)
        query = query.options(
            selectinload(Study.contributors),
...
class StudyService:
    ...
    async def get_by_identifier(self, study_identifier: str) -> Study:
        ...
        res = await self.session.exec(
            select(Study)
            .where(Study.identifier == study_identifier)
            .options(
                selectinload(Study.contributors),
                selectinload(Study.buildings),
                selectinload(Study.instruments),
    ...
    async def delete(self, study_id: int) -> Study:
        ...
        res = await self.session.exec(select(Study).where(Study.id == study_id))
        study = res.one_or_none()
    ...
    async def find(self, filter: dict, sort: list, range: list) -> StudiesResult:
        ...
        builder = StudyQueryBuilder(
            Study, filter, sort, range, {"$building": Building, "$space": Space}
        )
        ...
        return StudiesResult(
            total=total_count, skip=start, limit=end - start + 1, data=studies
        )

    async def save(self, study_draft: StudyDraft) -> Study:
        """Save the study draft in the database, replacing the existing study
        if one with same identifier exists."""
        ...
        contributors = [
            Person(
                **contributor_draft.model_dump(exclude={"id", "study_id"}),
                id=None,
                study_id=study.id,
            )
        ...
        for building_draft in building_drafts:
            ...
            for certification_draft in certification_drafts:
                certification = Certification(**certification_draft.model_dump())
            ...
            for certification in certifications:
                ...
                building_dict["certifications"].append(certification.model_dump())
        ...
        for dataset_draft in datasets_drafts:
            ...
            if dataset_draft.folder:
                new_folder = dataset_draft.folder["path"].replace("/draft/", "/pub/")
                dataset.folder["path"] = new_folder
                children = []
                for child in dataset_draft.folder["children"]:
                    new_child = child.copy()
                    new_child["path"] = new_child["path"].replace("/draft/", "/pub/")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            await s3_client.upload_local_file(
                temp_dir, "study.json", s3_folder=pub_folder
            )
    ...
    async def count_group_by(self, filter: dict, group_by: str) -> dict:
        ...
        builder = StudyQueryBuilder(
            Study, filter, [], [], {"$building": Building, "$space": Space}
        )
